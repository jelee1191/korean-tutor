...
export default function PracticeClient({ vocabulary }: PracticeClientProps) {
  ...
  useEffect(() => {
    ...
    const validVocabIds = new Set(vocabulary.map(w => w.id))

    // Filter out any due words that no longer exist in vocabulary
    const validDueWords = dueWords.filter(id => validVocabIds.has(id))
    ...
    if (validDueWords.length === 0) {
      ...
      const wordsToReview = [...validDueWords]
      if (wordsToReview.length < 10) {
        const newWords = vocabulary
          .filter(w => !validDueWords.includes(w.id))
          .sort(() => Math.random() - 0.5)
  ...
  const currentWord = vocabulary.find(w => w.id === sessionWords[currentIndex])
